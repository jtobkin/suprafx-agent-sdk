...
// Safety: this agent submits RFQs (locks funds) and accepts quotes
// (settles real assets). It runs in DRY_RUN unless LIVE=1 is set
// explicitly. In DRY_RUN it observes the live book and logs every
// decision it WOULD make, but never signs, submits, or accepts.
...
async function main() {
  console.log(
    DRY_RUN
      ? `[arb-taker] DRY RUN — observing the live book, no signing/submitting. Set LIVE=1 to trade with real funds.`
      : `[arb-taker] LIVE — will submit an RFQ and accept quotes with REAL funds.`,
  );
  if (signer) await signer.loadSequenceFromChain();
  console.log(
    `[arb-taker] delegate=${signer?.addressHex ?? "(none — dry run)"} pair=${PAIR} size=${SIZE} ref=${REF_RATE} min_edge=${MIN_EDGE_BPS}bps`,
  );
  ...
  // 1. Submit the RFQ (skipped in DRY_RUN — this locks sell-asset funds).
  const rfqIdBytes = randomBytes16();
  const rfqIdHex = bytesToUuid(rfqIdBytes);
  if (DRY_RUN) {
    console.log(
      `[arb-taker] DRY RUN: would submit rfq ${rfqIdHex.slice(0, 8)}… sell ${SIZE} ${SELL_TOKEN} for ${BUY_TOKEN} @ ref ${REF_RATE} (locks funds). Skipping.`,
    );
  } else {
  ...
  es.addEventListener("quote_placed", async (ev: { data: string }) => {
    ...
    // LIVE: only quotes on our own RFQ. DRY_RUN: observe every quote on
    // the pair so you can watch the edge logic against the real book.
    if (!DRY_RUN && q.rfq_id !== rfqIdHex) return;
    ...
    if (edgeBps >= MIN_EDGE_BPS) {
      if (DRY_RUN) {
        console.log(
          `[arb-taker] DRY RUN: WOULD ACCEPT quote ${q.id.slice(0, 8)} @ ${rate.toFixed(4)} (edge ${edgeBps.toFixed(1)}bps) — not accepting.`,
        );
        return;
      }
      console.log(
  ...
  setTimeout(async () => {
    if (accepted) return;
    if (DRY_RUN) {
      console.log(`[arb-taker] DRY RUN: 5 min elapsed; would cancel rfq. Exiting.`);
      es.close();
      process.exit(0);
    }
